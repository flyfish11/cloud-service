...
public class SysRoleServiceImpl implements SysRoleService {
	...
	public void setPermissionToRole(String roleId, Set<String> permissionIds) {
		...
		// 查出角色对应的old权限
		Set<String> oldPermissionIds = rolePermissionDao.findPermissionsByRoleIds(Sets.newHashSet(roleId)).stream()
				.map(p -> p.getId()).collect(Collectors.toSet());

		// 需要添加的权限
		Collection<String> addPermissionIds = org.apache.commons.collections4.CollectionUtils.subtract(permissionIds,
				oldPermissionIds);
		...
		// 需要移除的权限
		Collection<String> deletePermissionIds = org.apache.commons.collections4.CollectionUtils
	...
	@Override
	public Set<SysPermission> findPermissionsByRoleId(String roleId) {
