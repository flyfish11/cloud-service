...
//		parameters.put("username", username);
        ...
        saveLoginLog(account, "用户名密码登陆");

        return tokenInfo;
    }

    @PostMapping("/sys/adLogin")
    public Object adLogin(String account, String password) {
        Map<String, String> parameters = new HashMap<>();
        parameters.put("account" ,account);
        parameters.put("password", password);

        Object tokenInfo = oauth2Client.adLlogin(parameters);
        saveLoginLog(account, "ad域认证登录");
