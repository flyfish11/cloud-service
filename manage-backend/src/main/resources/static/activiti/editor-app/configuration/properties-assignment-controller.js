...
var KisBpmChoseAssignmentCtrl = ['$scope', '$http', function($scope, $http) {
    ...
    $scope.getAllRoles = function (successCallback) {
        $http({
            ...
            url: 'http://10.130.86.65:80/role/list'})
            .success(function (data, status, headers, config) {
               console.log(data)
                for (var i=0; i<data.length; i++) {
                    ...
                    roles.push({id:data[i].id,name:data[i].name});
                }
                $scope.roles = roles;
            })
    ...
    function getDetail(value,page,limit)
    {
        ...
        $http({
            ...
            headers: {'Accept': 'application/json',
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
            url: 'http://10.130.86.65:80/mgr/listByRoleId?roleId='+value+'&page='+page+'&limit='+limit})
            .success(function (data, status, headers, config) {
                console.log(data);
               ...
               //封装数据
                ...
                if (data.rows!= null) {
                    ...
                    for (var i=0; i<data.rows.length; i++) {
                        accounts.push({id:data.rows[i].id, code : data.rows[i].account, name : data.rows[i].name, index:i});
                    }
